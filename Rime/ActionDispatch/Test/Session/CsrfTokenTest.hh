...
class CsrfTokenTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->phpfunc = new \Rime\ActionDispatch\Test\MockPhpfunc;

        $this->session = new Session\User\Session(
            new Session\Factory\SegmentFactory,
            new Session\Factory\CsrfTokenFactory(new Session\Random\Randval($this->phpfunc)),
            $this->phpfunc,
            $_COOKIE
        );
    }
    ...
    public function testGetAndRegenerateValue()
    {
        ...
        $this->setExpectedException('\Rime\ActionDispatch\Session\Exception\Exception');
        $token->regenerateValue();
    }
